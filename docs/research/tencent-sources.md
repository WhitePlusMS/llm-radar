# Tencent 大模型来源追溯

## 2026-07-07 补充核验结论

- `Hunyuan-A13B`
  - 发布日期已按腾讯云官方产品动态收敛为 `2025-06-25`，不再保留 `2025-06-27`。
  - 由于官方描述明确它是“混元第一个混合推理模型”，模型卡已补入 `reasoning` 标签。
- `Tencent HY 2.0 Think`
  - 已补入官方主线模型卡。
  - 官方产品动态明确给出发布时间 `2025-11-09`，并说明底座已从 TurboS 升级到混元 2.0。
- `Tencent HY 2.0 Instruct`
  - 已补入官方主线模型卡。
  - 官方产品动态明确给出发布时间 `2025-11-11`。
- `Tencent HY 2.0 Think / Instruct`
  - 当前只用官方产品动态落最小事实集：型号存在、发布日期、文本输入输出、产品定位。
  - 由于页面未稳定公开参数、context window 与本项目 metrics.yaml 同口径 benchmark，本轮不猜这些字段。

## 已收录模型

### 1. Hunyuan-Large
- **发布日期**：2024-11-05
- **原始来源**：
  - 论文：Hunyuan-Large: An Open-Source MoE Model with 52 Billion Activated Parameters by Tencent
  - URL：https://arxiv.org/abs/2411.02265
  - 类型：arXiv 论文（发布方原始来源）
- **说明**：该论文为 Tencent Hunyuan 团队官方发布，论文中 Table 3 提供了预训练模型（pre-trained）的完整 benchmark 数据，包括 MMLU、MMLU-Pro、BBH、DROP、HellaSwag、ARC-C、WinoGrande、MATH、GSM8K、HumanEval 等。论文同时发布了代码与 checkpoint，因此标记为 open-weights。仓库当前模型卡使用 `2024-11-05` 作为 GitHub/开源发布日期；`2024-11-04` 可视为 arXiv 提交日。

### 2. Hunyuan-TurboS
- **发布日期**：2025-05-21（技术报告公开日期）
- **原始来源**：
  - 论文：Hunyuan-TurboS: Advancing Large Language Models through Mamba-Transformer Synergy and Adaptive Chain-of-Thought
  - URL：https://arxiv.org/abs/2505.15431
  - 类型：arXiv 论文（发布方原始来源）
- **说明**：该论文为 Tencent Hunyuan 团队官方发布，论文中 Table 2 提供了预训练模型（pre-trained）的 benchmark 数据，包括 MMLU、MMLU-Pro、BBH、DROP、HellaSwag、ARC-C、WinoGrande、MATH、GSM8K 等；摘要中给出了 LMSYS Chatbot Arena 分数 1356。模型采用 Hybrid Transformer-Mamba MoE 架构，总参数量 560B、激活参数量 56B，上下文窗口 256K。目前未找到公开的权重下载入口，标记为 closed-weights。

### 3. Hunyuan-T1
- **发布日期**：2025-03-21（按官方 GitHub README 首发记录收敛）
- **原始来源**：
  - 官方博客/README：混元-T1：强化学习驱动，业内首个超大规模混合Mamba推理模型正式发布
  - URL：https://github.com/Tencent/llm.hunyuan.T1/blob/main/README.md
  - 类型：官方 GitHub README / 博客（发布方原始来源）
- **说明**：Hunyuan-T1 是基于 Hunyuan-TurboS 基座训练的大型推理模型，官方 README 中给出了 MMLU-Pro 87.2、GPQA-diamond 69.3、MATH-500 96.2 等分数。目前未找到 arXiv 论文或 Hugging Face 权重仓库，标记为 closed-weights、reasoning。

### 4. HunyuanVideo-1.5
- **发布日期**：2025-11-20
- **原始来源**：
  - 官方 GitHub README：Tencent-Hunyuan/HunyuanVideo-1.5
  - URL：https://github.com/Tencent-Hunyuan/HunyuanVideo-1.5
  - 类型：官方代码仓库（发布方原始来源）
- **说明**：官方 README 明确给出 HunyuanVideo-1.5 为 `8.3B` 参数、开源视频生成模型，并说明支持 text-to-video 与 image-to-video。仓库还给出 `DiT + 3D causal VAE` 的技术定位。官方未披露 `active params`，因此模型卡中应显式保持 `active: undisclosed` 或留空。由于当前项目 metrics 主要是文本 benchmark，故保持 `scores: {}`。

### 5. Tencent HY 2.0 Think
- **发布日期**：2025-11-09
- **原始来源**：
  - 腾讯云官方产品动态：腾讯云大模型知识引擎产品动态
  - URL：https://cloud.tencent.com/document/product/1729/97765
  - 类型：官方产品文档（发布方原始来源）
- **说明**：官方动态明确写出 `Tencent HY 2.0 Think` 已上线，且底座从 TurboS 升级到混元 2.0，重点增强复杂指令遵循、多轮与长文理解、代码、Agent、推理能力。当前页面未稳定公开参数与上下文窗口。

### 6. Tencent HY 2.0 Instruct
- **发布日期**：2025-11-11
- **原始来源**：
  - 腾讯云官方产品动态：腾讯云大模型知识引擎产品动态
  - URL：https://cloud.tencent.com/document/product/1729/97765
  - 类型：官方产品文档（发布方原始来源）
- **说明**：官方动态明确写出 `Tencent HY 2.0 Instruct` 已上线，底座从 TurboS 升级到混元 2.0，重点增强指令遵循、多轮与长文理解、文学创作、知识准确性、代码及推理能力。当前页面未稳定公开参数与上下文窗口。

## 未找到来源的模型/分数

- **Hunyuan 3.0**：2026 年 3 月腾讯财报会上提到将于 2026 年 4 月发布，目前尚未正式发布，无原始技术报告或论文，因此未收录。
- **swe-bench 分数**：三个模型均未在原始来源中报告 SWE-bench 分数，故未填写。
- **Hunyuan-TurboS / Hunyuan-T1 的 HumanEval 分数**：TurboS 论文 Table 2 未包含 HumanEval；T1 官方 README 未报告 HumanEval，故未填写。
- **Hunyuan-T1 的 ifeval、bbh、drop、hellaswag、arc-c、winogrande、gsm8k 等分数**：官方 README 未报告这些 benchmark 分数，故未填写。
- **HunyuanVideo-1.5 的文本 benchmark**：官方 README 未给出本项目当前 `metrics.yaml` 同口径分数，且模型定位为视频生成，因此未填写。

## 参考来源列表（原始来源优先）

1. https://arxiv.org/abs/2411.02265
2. https://arxiv.org/pdf/2411.02265.pdf
3. https://arxiv.org/abs/2505.15431
4. https://arxiv.org/pdf/2505.15431.pdf
5. https://github.com/Tencent/llm.hunyuan.T1/blob/main/README.md
6. https://github.com/Tencent-Hunyuan/HunyuanVideo-1.5
